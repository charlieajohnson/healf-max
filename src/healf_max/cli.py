...
def kb_validate() -> None:
    """Validate markdown KB records."""
    settings = load_settings()
    result = validate_kb(settings.kb_dir)
    if result.ok:
...
def kb_search(query: str = typer.Argument(..., help="Search query.")) -> None:
    ...
    settings = load_settings()
    records = search_kb(query, kb_dir=settings.kb_dir, storage_dir=settings.storage_dir)
    if not records:
        console.print("[yellow]No matching KB records found.[/yellow]")
        return

    table = Table(title="KB search")
    table.add_column("score", justify="right")
    table.add_column("kind")
    table.add_column("id")
    table.add_column("title")
    for record in records:
        table.add_row(f"{record.score:.1f}", record.kind, record.id, record.title)
    console.print(table)
